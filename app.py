...
async def cater_add(ack, body, client, view):
    ...
    if not re.match(regex, cater_phone):
        errors['block_phone'] = "Please enter a valid, 10 digit phone number"
    if len(errors) > 0:
...
async def add_trello(ack, body, client):
    # TODO Remove Trello functions, add todoist if needed
...
async def waste_view(ack, body, client):
